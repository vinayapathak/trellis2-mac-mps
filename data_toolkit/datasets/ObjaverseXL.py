# Ported verbatim from microsoft/TRELLIS's dataset_toolkits/datasets/ObjaverseXL.py -- TRELLIS.2's
# own data_toolkit/ (this repo and the upstream microsoft/TRELLIS.2 repo, confirmed identical via
# the GitHub API) documents a `datasets.<SUBSET>` plugin interface (add_args/get_metadata/download/
# foreach_instance) but never ships the actual per-dataset connector modules -- they only exist in
# the original TRELLIS v1 repo. Confirmed this gap is real (not a local checkout issue) before
# porting anything.
import os
import argparse
...
def download(metadata, download_root, **kwargs):
    # v1's dataset_toolkits called this kwarg `output_dir`; TRELLIS.2's data_toolkit consistently
    # calls it `download_root` (confirmed against dump_mesh.py's/dump_pbr.py's own foreach_instance
    # call sites) and download.py invokes this function via `**opt`, so the parameter name here has
    # to match exactly, not just be renamed cosmetically.
